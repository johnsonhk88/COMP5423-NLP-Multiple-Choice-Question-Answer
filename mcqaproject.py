# -*- coding: utf-8 -*-
"""
Created on Wed Apr 14 11:38:28 2021
...
class Race2(object):
    ...
    def __repr__(self):
        l = [
            ...
            "context_sentence: {}".format(self.context_sentence),
            "start_ending: {}".format(self.start_ending),
            "ending_0: {}".format(self.ending_0),
            "ending_1: {}".format(self.ending_1),
            "ending_2: {}".format(self.ending_2),
            "ending_3: {}".format(self.ending_3),
        ]
...
if __name__ == "__main__":
    ...
    eval_batch_size = 3
    numEpoch = 2
    maxNumFileForTestDirectory = 200
    maxNumFileforTrainDirectory = 300
    maxNumFileForEvalDirectory = 200
    ...
    #getTrain sample 
    trainSamples =  mcaqTrainModel.read_raceModify(race_raw_train_path, maxNumFileforTrainDirectory)
    ...
    evalSamples =  mcaqTrainModel.read_raceModify(race_raw_dev_path, maxNumFileForEvalDirectory)
    evalFeatures = mcaqTrainModel.convert_examples_to_features1(evalSamples, tokenizer, max_seq_length, True)
    print("\n***** Running evaluation *****")
    print("  Num examples = {}".format(len(evalSamples)))
    print("  Batch size = {}".format(eval_batch_size))
    
    #convert into tensor
    all_input_ids =  torch.tensor(mcaqTrainModel.selectField(evalFeatures, 'input_ids') ,dtype=torch.long)
    all_input_mask = torch.tensor(mcaqTrainModel.selectField(evalFeatures, 'input_mask') ,dtype=torch.long)
    all_segment_ids =  torch.tensor(mcaqTrainModel.selectField(evalFeatures, 'segment_ids') ,dtype=torch.long)
    ...
    evalDataLoader = DataLoader(evalData, sampler=eval_sampler, batch_size= eval_batch_size)
    ...
    total_logits = []
    total_labels = []
    for input_ids, input_mask, segment_ids, label_ids in tqdm(evalDataLoader, desc="Evaluating"):
        ...
        tmp_eval_accuracy = mcaqTrainModel.accuracy(logits, label_ids)
